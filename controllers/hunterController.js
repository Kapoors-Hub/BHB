...
// Get Fouls
  ...
  async getMyFouls(req, res) {
    try {
      ...
      // Calculate statistics using reduce for better performance
      const { totalXpPenalty, severityCounts } = foulRecords.reduce(
        (stats, record) => {
          // Add to total XP penalty
          stats.totalXpPenalty += record.xpPenalty || 0;
          
          // Count by severity
          if (record.foul && record.foul.severity) {
            stats.severityCounts[record.foul.severity]++;
          }
      ...
      console.error('Error in getMyFouls:', error);
      return res.status(500).json({
        status: 500,
        success: false,
        message: 'Error retrieving foul records',
